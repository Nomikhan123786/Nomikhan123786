# 👋 Hi, I'm Noman Nawaz Khan

Passionate **Full-Stack Web Developer** specializing in the **MERN Stack** (MongoDB, Express.js, React.js, Node.js). I love building fast, scalable, and user-friendly web applications.

🔗 **Portfolio:** None
💼 **LinkedIn:** https://www.linkedin.com/in/noman-nawaz-khan-305557283?utm_source=share_via&utm_content=profile&utm_medium=member_android
📧 **Email:** nomank7887682@gmail.com

---

## 🔧 What I Do

- 🚀 Build fast, responsive, and dynamic websites & dashboards
- 🔌 Design RESTful APIs and real-time applications
- 🖥️ End-to-end solutions using MongoDB, Express, React, Node
- 🎨 Focused on clean UI/UX and performance optimization

---

## 🛠️ Tech Stack & Tools

**Languages & Frameworks**
`JavaScript` `TypeScript` `Node.js` `React.js` `Express.js`  `HTML` `CSS` `Tailwind CSS` `Bootstrap`

**Databases & APIs**
`MongoDB` `Mongoose`  `REST APIs` 

**Tools & Platforms**
`Git` `GitHub` `VS Code`   `Vercel` 

---

## 🌐 Projects & Links

- [SwiftMove_Clone]( https://swift-move-clone.vercel.app/)
- [Task Manager](https://task-manager-three-psi-58.vercel.app/)

---

⭐ **Feel free to explore my repos, contribute, or reach out for collaboration!**
